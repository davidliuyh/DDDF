...
DEFAULT_QUIJOTE_ROOT = Path("/global/homes/l/liuyh15/WorkSpace/Quijote")
DEFAULT_2LPTIC_DIR_CANDIDATES = (
    Path("~/WorkSpace/Quijote/2lpt").expanduser(),
    Path("~/WorkSpace/Quijote/2lpt/2LPTic").expanduser(),
    Path("/global/homes/l/liuyh15/Workspace/DDDF/2LPTic"),
    Path("/global/homes/l/liuyh15/WorkSpace/DDDF/2LPTic"),
)
...
def _set_or_append_param(lines_in: Sequence[str], key: str, value: str) -> List[str]:
    out: List[str] = []
    found = False
...
def _get_param_value(lines_in: Sequence[str], key: str) -> Optional[str]:
    for line in lines_in:
        stripped = line.strip()
...
def _resolve_resource_path(src_param: Path, raw_value: str, quijote_root: Optional[Path]) -> Path:
    p = Path(raw_value).expanduser()
    if p.is_absolute() and p.exists():
        return p

    candidates = [(src_param.parent / p).resolve()]
    if quijote_root is not None:
        root = Path(quijote_root).expanduser()
        candidates.extend(
            [
                (root / p).resolve(),
                (root / "2lpt" / p).resolve(),
                (root / "CAMB_TABLES" / p.name).resolve(),
                (root / "2lpt" / "GLASS" / p.name).resolve(),
            ]
        )
...
def resolve_2lptic_dir(lptic_dir: Optional[Path] = None) -> Path:
    repo_candidate = Path(__file__).resolve().parents[1] / "2LPTic"

    candidates: List[Path] = []
    if lptic_dir is not None:
        candidates.append(Path(lptic_dir).expanduser())
    candidates.extend(DEFAULT_2LPTIC_DIR_CANDIDATES)
    candidates.append(repo_candidate)

    normalized: List[Path] = []
    for cand in candidates:
        # Accept either ".../2lpt" (directory) or ".../2lpt/2LPTic" (binary path).
        if cand.name == "2LPTic" and not cand.is_dir():
            cand = cand.parent
        if cand not in normalized:
            normalized.append(cand)

    found = _first_existing_path([c for c in normalized if c.is_dir()])
    if found is None:
        all_candidates = ", ".join(str(c) for c in normalized)
        raise FileNotFoundError(f"Unable to locate 2LPTic directory. Tried: {all_candidates}")
    return found


def ensure_2lptic_executable(
    lptic_dir: Optional[Path] = None,
    force_rebuild: bool = False,
) -> Path:
    resolved_dir = resolve_2lptic_dir(lptic_dir=lptic_dir)
    executable = resolved_dir / "2LPTic"

    if force_rebuild and executable.exists():
        executable.unlink()
    ...
    if build.returncode != 0:
        msg = build.stderr.strip() or build.stdout.strip() or "unknown make error"
        raise RuntimeError(f"Failed to build 2LPTic in {resolved_dir}: {msg}")

    if not executable.exists():
        raise FileNotFoundError(
            f"2LPTic executable was not produced after make in {resolved_dir}"
        )
...
def list_available_seeds(root_base: Path) -> List[int]:
    seeds: List[int] = []
    for p in sorted(Path(root_base).iterdir()):
...
def write_wn_param_file(
    seed_root: Path,
    quijote_root: Optional[Path] = DEFAULT_QUIJOTE_ROOT,
    output_name: str = "2LPT_wn.param",
) -> Path:
    seed_root = Path(seed_root).expanduser()
    src_param = seed_root / "ICs" / "2LPT.param"
    ...
    dst_param = wn_dir / output_name
    white_noise_file = wn_dir / "white_noise.npz"

    lines = src_param.read_text(encoding="utf-8").replace("\r\n", "\n").splitlines()
    lines = _set_or_append_param(lines, "OutputDir", str(wn_dir))
    lines = _set_or_append_param(lines, "FileBase", "ics")
    lines = _set_or_append_param(lines, "UseWhiteNoiseInput", "0")
    lines = _set_or_append_param(lines, "ExportWhiteNoiseOnly", "1")
    lines = _set_or_append_param(lines, "WhiteNoiseFile", str(white_noise_file))

    for key in ("FileWithInputSpectrum", "GlassFile"):
        raw_value = _get_param_value(lines, key)
        if raw_value is not None:
            resolved = _resolve_resource_path(src_param, raw_value, quijote_root)
            lines = _set_or_append_param(lines, key, str(resolved))

    dst_param.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return dst_param


def generate_white_noise_for_root(
    seed_root: Path,
    lptic_dir: Optional[Path] = None,
    quijote_root: Optional[Path] = DEFAULT_QUIJOTE_ROOT,
    force_overwrite: bool = False,
    skip_existing: bool = True,
    reset_wn_dir: bool = False,
) -> Path:
    seed_root = Path(seed_root).expanduser()
    wn_dir = seed_root / "wn"
    ...
    if reset_wn_dir and wn_dir.exists():
        shutil.rmtree(wn_dir)
    wn_dir.mkdir(parents=True, exist_ok=True)

    if skip_existing and white_noise_file.exists() and dst_param.exists():
        return white_noise_file

    dst_param = write_wn_param_file(
        seed_root=seed_root,
        quijote_root=quijote_root,
        output_name="2LPT_wn.param",
    )

    if force_overwrite and white_noise_file.exists():
        white_noise_file.unlink()

    lptic_exe = ensure_2lptic_executable(lptic_dir=lptic_dir)
    env = dict(os.environ)
    env["MPICH_GPU_SUPPORT_ENABLED"] = "0"

    result = subprocess.run(
        [str(lptic_exe), str(dst_param)],
        cwd=str(wn_dir),
        env=env,
        capture_output=True,
        text=True,
    ...
    if result.returncode != 0:
        msg = result.stderr.strip() or result.stdout.strip() or "2LPTic failed without output"
        raise RuntimeError(f"2LPTic failed for seed_root={seed_root}: {msg}")

    if not white_noise_file.exists():
        raise FileNotFoundError(f"white_noise.npz was not generated: {white_noise_file}")
    return white_noise_file


def generate_white_noise_for_seed(
    seed: int,
    catalog_name: str = "fiducial_LR",
    quijote_base: Path = Path("/pscratch/sd/l/liuyh15/Quijote"),
    lptic_dir: Optional[Path] = None,
    quijote_root: Optional[Path] = DEFAULT_QUIJOTE_ROOT,
    force_overwrite: bool = False,
    skip_existing: bool = True,
    ...
    seed_root = Path(quijote_base).expanduser() / catalog_name / str(seed)
    return generate_white_noise_for_root(
        seed_root=seed_root,
        lptic_dir=lptic_dir,
        quijote_root=quijote_root,
        force_overwrite=force_overwrite,
        skip_existing=skip_existing,
        reset_wn_dir=reset_wn_dir,
    )


def find_seed_psi1_qinit_files(wn_dir: Path, seed: int) -> Tuple[Optional[Path], Optional[Path]]:
    tag = f"seed{seed:03d}"
    wn_dir = Path(wn_dir)
    psi1_candidates = sorted(wn_dir.glob(f"psi1_grid_z127_{tag}_n*.npz"))
    qinit_candidates = sorted(wn_dir.glob(f"q_init_{tag}_n*.npz"))
    psi1_file = psi1_candidates[-1] if psi1_candidates else None
    qinit_file = qinit_candidates[-1] if qinit_candidates else None
