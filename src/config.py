...
Runtime settings (realization, N_p, filter_dir) are set in the notebook;
...
def ensure_filter_dirs(filter_dir):
    """Ensure data/model subdirectories for the active filter exist."""
    os.makedirs(f'{data_path}/{filter_dir}', exist_ok=True)
    os.makedirs(f'{model_path}/{filter_dir}', exist_ok=True)
...
# Active subdir and hyperparameters are always sourced from the selected preset.
filter_dir            = active_gan_defaults['filter_dir']
...
ensure_filter_dirs(filter_dir)

# ── Output path helpers (match existing filenames on disk) ────────────────────
def best_fit_coef_path(realization, filter_dir=filter_dir, L=L, N_p=N_p):
    return f'{data_path}/{filter_dir}/best-fit-coefL{L}N{N_p}-{realization}.npz'

def best_fit_avg_coef_path(filter_dir=filter_dir, L=L, N_p=N_p):
    return f'{data_path}/{filter_dir}/best-fit-coef-avgL{L}N{N_p}.npz'

def best_fit_path(realization, filter_dir=filter_dir, L=L, N_p=N_p):
    return f'{data_path}/{filter_dir}/best-fitL{L}N{N_p}-{realization}.npz'

def best_fit_delta_path(realization, filter_dir=filter_dir, L=L, N_p=N_p):
    return f'{data_path}/{filter_dir}/best-fit-deltaL{L}N{N_p}-{realization}.npz'

def target_delta_path(realization, filter_dir=filter_dir, L=L, N_p=N_p):
    return f'{data_path}/{filter_dir}/target-deltaL{L}N{N_p}-{realization}.npz'
...
def training_data_path(realizations, ps=patch_size, p=padding, ov=overlap,
                       rot=rotate, N_p=N_p, filter_dir=filter_dir):
    tag = f'IC2RES-N{N_p}PS{ps}P{p}O{int(100 * ov):02d}Rotate{rot}-{realization_tag(realizations)}'
    return f'{data_path}/{filter_dir}/training-data-{tag}.npz'

def unet_model_name(realizations, ps=patch_size, p=padding,
                    rot=rotate, N_p=N_p, filter_dir=filter_dir):
    rtag = realization_tag(realizations)
    return f'{model_path}/{filter_dir}/unet-IC2RES-N{N_p}PS{ps}P{p}Rotate{rot}-{rtag}'


def gan_model_name(realizations, ps=patch_size, p=padding,
                   rot=rotate, N_p=N_p, filter_dir=filter_dir):
    rtag = realization_tag(realizations)
    return f'{model_path}/{filter_dir}/gan-IC2RES-N{N_p}PS{ps}P{p}Rotate{rot}-{rtag}'
