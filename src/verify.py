...
def verify_realization(
    ...
    # ── Overdensity slices ────────────────────────────────────────────────
    slice_labels = ["N-body", "Baseline", "Baseline + IC2RES", "Residual to N-body"]
    ...
    if bispectrum:
        ...
        bks = [PKL.Bk(d, boxsize, k1_bk, k2_bk, theta, MAS=MAS, threads=threads) for d in deltas]
        ...
        ax.set_xlim(0, np.pi)
        ax.set_ylim(0, 1.5)
        ax.set_title(f"[r{realization}] Bispectrum z=0 (WN), k1={k1_bk}, k2={k2_bk}")
