"""
Configuration for the ZA-baseline WN IC→Residual pipeline.

Self-contained and isolated from the non-ZA workflow.
"""
...
data_dir   = 'new_tophat0.4_ZA'
model_dir  = 'new_tophat0.4_ZA_v3'
...
def best_fit_coef_path(realization, data_dir=None, L=L, N_p=N_p, filter_dir=None):
    ...
    return f'{data_path}/{ddir}/za-baseline-coefL{L}N{N_p}-{realization}.npz'


def best_fit_avg_coef_path(data_dir=None, L=L, N_p=N_p, filter_dir=None):
    ddir = _resolve_data_dir(data_dir=data_dir, filter_dir=filter_dir)
    return f'{data_path}/{ddir}/za-baseline-coef-avgL{L}N{N_p}.npz'


def best_fit_path(realization, data_dir=None, L=L, N_p=N_p, filter_dir=None):
    ddir = _resolve_data_dir(data_dir=data_dir, filter_dir=filter_dir)
    return f'{data_path}/{ddir}/za-baselineL{L}N{N_p}-{realization}.npz'


def best_fit_delta_path(realization, data_dir=None, L=L, N_p=N_p, filter_dir=None):
    ddir = _resolve_data_dir(data_dir=data_dir, filter_dir=filter_dir)
    return f'{data_path}/{ddir}/za-baseline-deltaL{L}N{N_p}-{realization}.npz'
...
vec_gan_v1 = {
    'vec_data_dir': 'za_psi_vec',
    'vec_model_dir': 'za_psi_vec_v1',
    'vec_batch_size': 512,
    'vec_rotate': False,
