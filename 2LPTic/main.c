...
void displacement_fields(void)
{
  ...
  double delta, phase, phase_shift, ampl, hubble_a;
	double mode_re, mode_im;
  double u, v, w;
  double f1, f2, f3, f4, f5, f6, f7, f8;
  double dis, dis2, maxdisp, max_disp_glob;
	unsigned int *seedtable = NULL;
	void *white_noise_stream = NULL;
	float *white_noise_row = NULL;
	char input_white_noise_path[600];
  ...
  use_white_noise = (UseWhiteNoiseInput == 1);

	if(ExportWhiteNoiseOnly == 1 && use_white_noise)
		{
			if(ThisTask == 0)
				printf("ExportWhiteNoiseOnly requires UseWhiteNoiseInput = 0.\n");
			FatalError(916);
		}

  if(use_white_noise)
    {
      if(WhiteNoiseFile[0] != 0)
	snprintf(input_white_noise_path, sizeof(input_white_noise_path), "%s", WhiteNoiseFile);
      else
	snprintf(input_white_noise_path, sizeof(input_white_noise_path), "%s/white_noise.npz", OutputDir);

      rc = open_white_noise_npz_stream(input_white_noise_path, Nmesh, &white_noise_stream);
      if(rc != 0)
	{
	  if(ThisTask == 0)
	    printf("Failed to open white noise input npz: %s\n", input_white_noise_path);
	  FatalError(913);
	}

      white_noise_row = (float *) malloc(sizeof(float) * (size_t) (2 * (Nmesh / 2 + 1)));
      if(white_noise_row == NULL)
	FatalError(914);

      if(ThisTask == 0)
	printf("Using input white noise from %s\n", input_white_noise_path);
    }
  else
    {
      ...
      for(i = 0; i < Nmesh / 2; i++)
	{
      ...
      if(ThisTask == 0)
	{
	  char white_noise_path[600];

	  snprintf(white_noise_path, sizeof(white_noise_path), "%s/white_noise.npz", OutputDir);
	  rc = write_white_noise_npz(white_noise_path,
				      seedtable,
				      Nmesh,
				      Nsample,
				      Seed,
          ...
          if(rc != 0)
	    {
	      printf("Failed to write white noise npz: %s\n", white_noise_path);
	      FatalError(912);
	    }
	  printf("Saved white noise to %s\n", white_noise_path);
	}

				if(ExportWhiteNoiseOnly == 1)
					{
						if(seedtable)
							free(seedtable);
						if(random_generator)
						...
						MPI_Barrier(MPI_COMM_WORLD);
						if(ThisTask == 0)
							printf("ExportWhiteNoiseOnly=1: exit after white noise export.\n");
						MPI_Finalize();
						exit(0);
					}
    }
	  ...
	  for(i = 0; i < Nmesh; i++)
	    {
	      for(j = 0; j < Nmesh; j++)
		{
		  rc = read_white_noise_npz_row(white_noise_stream, white_noise_row, Nmesh / 2 + 1);
		  if(rc != 0)
		    {
		      if(ThisTask == 0)
			printf("Failed while reading white noise row i=%d j=%d\n", i, j);
		      FatalError(915);
		    }
		  ...
		  for(k = 0; k <= Nmesh / 2; k++)
		    {
		      mode_re = white_noise_row[2 * k];
		      mode_im = white_noise_row[2 * k + 1];

		      if(mode_re == 0.0 && mode_im == 0.0)
			continue;
		      ...
		      for(axes = 0; axes < 3; axes++)
			{
			  cdisp[axes][((i - Local_x_start) * Nmesh + j) * (Nmesh / 2 + 1) + k].re =
			    -kvec[axes] / kmag2 * delta * mode_im;
			  cdisp[axes][((i - Local_x_start) * Nmesh + j) * (Nmesh / 2 + 1) + k].im =
			    kvec[axes] / kmag2 * delta * mode_re;
			}
		    }
		}
	    }
  ...
  for(axes = 0; axes < 3; axes++) free(cdisp2[axes]);
	...
	if(random_generator)
		gsl_rng_free(random_generator);
	if(white_noise_row)
	free(white_noise_row);
	if(white_noise_stream)
	close_white_noise_npz_stream(white_noise_stream);
